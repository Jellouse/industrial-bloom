...
- Intro transition: the shop’s “Explore available products” control fades over the first 120px of scroll and jumps to the numbered editions. The floating claim/cart control uses the same handoff progress but remains transparent until its entire box has cleared the launch hero. The editorial landing page still uses “Explore more below” for the long photographs.
...
website/                 editable static-site source
  hero.js                shared launch-hero video, scale, and newsletter signup
...
| Route | Purpose |
| --- | --- |
| `/` | Brand landing page, newsletter signup, long photographs, and shop entry |
| `/shop/` | Launch video hero, newsletter signup, and numbered editions with cart |
...
Newsletter signup remains on `/` and `/shop/`. Do not claim the shop is unlaunched. Both pages post to `/api/subscribe` through `website/hero.js`. The Vercel function validates email and adds or updates contacts in the configured Brevo list. Before meaningful traffic, add server-side rate limiting and bot protection.
