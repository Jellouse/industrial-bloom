...
`website/shop/shop.js` loads `/api/shop/products` and sorts the live catalog as Type 660, 120, 490, then 28. Type 660 is featured full-width; the others sit in a two-column shop. Each card is image-first: native-aspect gallery, Type kicker, catalog name with any live “Type” prefix stripped, next serial (`N° x of y`), live price, and Add / Choose on one footer. Description stays in the Choose panel. The series block is the four-type set: first-frame shop photographs, a combined price for one of each type still in stock, individual live prices, and Add / Choose. Galleries use an explicit index and horizontal transforms for swipe, drag, and keyboard navigation. After a multi-image product is in view for 2.4 seconds, its first image gives one subtle horizontal peek and returns; interaction cancels the hint, and reduced-motion users never receive it. The homepage (`/`) uses the same paper, type, and header language, with a framed launch video used only as a featured Type 660 still-motion well — not a full-viewport cinematic hero.
